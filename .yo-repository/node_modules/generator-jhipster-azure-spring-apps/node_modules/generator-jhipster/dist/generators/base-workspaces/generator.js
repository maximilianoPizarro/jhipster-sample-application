...
export default class BaseWorkspacesGenerator extends BaseGenerator {
        ...
        await this.prompt([
            ...
            },
            {
                ...
                when: async (answers) => {
                    ...
                    appsFolders = (await this.findApplicationFolders(directoryPath)).filter(app => app !== 'jhipster-registry' && app !== 'registry');
    ...
    async findApplicationFolders(directoryPath = this.directoryPath ?? '.') {
        return (await readdir(this.destinationPath(directoryPath), { withFileTypes: true }))
            ...
            .filter(folder => existsSync(this.destinationPath(directoryPath, folder, 'package.json')) &&
            existsSync(this.destinationPath(directoryPath, folder, YO_RC_FILE)));
    }
    async resolveApplicationFolders({ directoryPath = this.directoryPath, appsFolders = this.appsFolders ?? [], } = {}) {
    ...
    async bootstrapApplications() {
        ...
        this.getSharedApplication(this.destinationPath()).workspacesApplications = Object.entries(resolvedApplicationFolders).map(([appFolder, resolvedFolder], index) => {
