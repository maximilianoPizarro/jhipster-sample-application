...
export const jhipster7deprecatedProperties = {
    devDatabaseType: {
        ...
        get: ({ data }) => {
            if (data.devDatabaseType !== undefined)
                return data.devDatabaseType;
            const fallbackValue = data.prodDatabaseType ?? data.databaseType;
            console.log(`JHipster v8 behavior change(devDatabaseType is only used in jhipster:spring-data-relational generator): devDatabaseType is not set, using fallback: ${fallbackValue}`);
    ...
    prodDatabaseType: {
        ...
        get: ({ data }) => {
            if (data.prodDatabaseType !== undefined)
                return data.prodDatabaseType;
            console.log(`JHipster v8 behavior change(prodDatabaseType is only used in jhipster:spring-data-relational generator): devDatabaseType is not set, using fallback: ${data.databaseType}`);
